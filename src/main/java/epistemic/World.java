...
public class World extends HashMap<NormalizedWrappedLiteral, Set<NormalizedWrappedLiteral>> {
    ...
    /**
     * Adds to world valuation. Does not attempt to add the new value to the (super) mapping, since this is not generated by a range rule.
     * @param value
     */
    public void addToValuation(NormalizedWrappedLiteral value)
    {
        valuation.add(value);
        sortedPropNames.add(value.toSafePropName());
        updateHashCache();
    ...
    /**
     * Removes from world valuation. Does not attempt to remove the value from the (super) mapping, since this may not be generated by a range rule.
     * @param value
     */
    public boolean removeFromValuation(NormalizedWrappedLiteral value)
    {
        if(valuation.contains(value))
        {
            valuation.remove(value);
            sortedPropNames.remove(value.toSafePropName());
            updateHashCache();
            return true;
        }
        return false;
    ...
    public String getUniqueName() {
        return String.valueOf(getWorldIdStr());
    }
